...
import { S as Subscribable, p as pendingThenable, g as resolveEnabled, s as shallowEqualObjects, h as resolveStaleTime, n as noop, i as isServer, k as isValidTimeout, t as timeUntilStale, l as timeoutManager, o as focusManager, q as fetchState, v as replaceData, w as notifyManager, x as hashKey, y as getDefaultState, r as reactExports, z as shouldThrowError, A as useQueryClient, a as useInternetIdentity, B as createActorWithConfig } from "./index-kwy3CwmH.js";
...
function useInteraktWebhookUrl() {
  const { actor, isFetching } = useActor();
  return useQuery({
...
function useSubmitLead() {
  ...
  return useMutation({
    mutationFn: async (data) => {
      ...
      actor.triggerInteraktNotification(data.name, data.phone, data.email).catch(() => {
      });
      return id;
    }
  });
...
function useSetInteraktWebhookUrl() {
  const { actor } = useActor();
  const queryClient = useQueryClient();
  return useMutation({
...
export {
  ...
  useIsAdmin as c,
  useLeads as d,
  useUpdateLeadStatus as e,
  useDeleteLead as f,
  useInteraktWebhookUrl as g,
  useSetInteraktWebhookUrl as h,
  useAddGalleryItem as i,
  useUpdateGalleryItem as j,
  useRemoveGalleryItem as k,
